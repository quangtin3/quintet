...
(function($) {
    jQuery.fn.fontPickerRegios = function(settings) {
        ...
        return this.each(function() {
            ...
            var config = $.extend({
                defaultFont: 'Helvetica Neue', // default font to display in selector
            ...
            if (!fontPicker.length) {
                ...
                $(document.body).click(function(event) {
                    if ($(event.target).is('#' + config.selid) || $(event.target).is('#' + config.id))
                        return;
            ...
            /* If we already took care of the fontPicker, dont do this again */
            if (!fontPicker[0].innerHTML)
            {

                $.each(fonts, function(i, item) {

                    fontPicker.append('<div class="' + config.fontclass + '" onmouseover="this.style.backgroundColor=\'' + config.hoverColor
